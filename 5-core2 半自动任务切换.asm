...
section header vstart=0 align=16
    ...
    symbol_table_begin:
        print_info:  
                    dd (switch_task_string - print ) ;字符串长度,用于比较
                    dd print    ;字符串的偏移地址
                    dd 0        ;对应地址表的索引

        switch_task_info:
					dd (symbol_table_string_end - switch_task_string)
                    dd switch_task_string     ;字符串偏移地址
                    dd 1        ;索引
...
section code vstart=0 align=16
    ...
    ;使用switch_task 手动切换任务
    ;当switch_task 执行到 jmp 指令时, 保存用户程序上下文到TR指向的用户TSS结构
    ;把用户TSS描述符B=0, 内核TSS描述符B=1
    ;加载内核描述符到TR, 从内核TSS中恢复上一次内核保存的上下文, 也就是从eip=pop ds处执行
    call far [es:symbol_table_addr_begin + 8]
